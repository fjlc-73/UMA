...
\end{proposicion}
\begin{proof}
Usando las reglas de la definición 1.1 tenemos que:
\begin{multline*}
...
\end{multline*}
Al ser 
  $(\alpha+\beta)\gamma \text{ y } \alpha\gamma+\beta\gamma$ dos expresiones regulares que general el mismo lenguaje deducimos que son equivalentes.
  
\end{proof}

\begin{example}
Consideremos $L=\{w\in \{a,b\}^* : w \textnormal{ no termina en } ab\}$. Un expresión regular que genera L es: 
((a+b)*(aa+ba+bb))+(a+b+$\epsilon$)
